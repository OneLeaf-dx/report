...
## 2026-09-26 20:46 — Claude Code

- **範圍**：內容
- **摘要**：依使用者指名並指示以 PR 提出（不直接改 main），依〈論文修訂說明〉表 7-3 更正兩篇報告。〈消融報告〉：百分點差由「%」改為「個百分點」（§1、表 3-1、§4.1），「提升 2.7 倍」改為「為基線的 3.7 倍」（85.05／22.92），「茶樹葉枯病」改為與題庫一致的「茶樹輪斑病」，補回 §2.6 被吃掉的公式符號（S、s_i、C、G）與 Faithfulness 公式的數學環境；§4.4 第 2 點的第 39 題敘述待原始 CSV 確認，未修改。〈架構設計〉：知識庫大小由 30 KB 更正為約 19 KB（兩處），補回句尾截掉的字（診斷服務、公用函式、施藥禁忌）、5 處右括號與 2 處句尾冒號，開頭的「1. 系統總覽」改為不編號的前言以消除重號。〈論文修訂說明〉同步更新 X2、X5、X7 的狀態。兩支關卡腳本皆 PASS
- **影響檔案**：`植物病蟲害辨識 (115資工四A)/系統測試與評估/20260924_citrus_rag_ablation_study.md`、`植物病蟲害辨識 (115資工四A)/RAG向量資料庫/架構設計.md`、`植物病蟲害辨識 (115資工四A)/TAAI2026/論文修訂說明.md`
- **commit**：`8877e2d`
...
## 2026-09-14 14:17 — Claude Code

- **範圍**：內容
- **摘要**：新增根目錄 `SUMMARY.md` 成果總覽，給教授與評審快速看完整體成果，底層報告改為可查證的附錄。內容只從 7 篇報告摘錄（2026-09-13 週報、v5.7 訓練評估、v5.7 資料集統計、即時辨識門檻、三平台整合、行動端 RAG、RAGAs），分全案現況、影像辨識模型、資料集、行動端部署、RAG 與小語言模型、還開著的問題六段，每段附原報告連結與章節，不新增論述、不跨報告歸納。129 個數字逐一比對，都能在原報告找到原文。兩處加註經使用者同意保留：週報「部署端不採用即時辨識」註明只量過 Snapdragon 662；週報表 6-3「結論只成立於一台裝置」已由 2026-09-14 三平台報告量測，故不列。最新報告沒有圖，因此未引用已被取代的舊版圖。全文經 DreamOver9183 審閱核准後才 commit。同步修改：根目錄 README 路由表第一列加入口；`SPECIFICATION.md` §1 層級表加 L0 `SUMMARY.md` 一列，規則 2 加例外（可直接引用 L2 資產圖、不得複製）；`add-report` 加一條：新報告取代 SUMMARY 引用的結論時要同步更新並經人類核准。未動任何 L2 報告正文
- **影響檔案**：`SUMMARY.md`（新增）、`README.md`、`.agent/SPECIFICATION.md`、`.agent/skills/add-report/SKILL.md`
- **commit**：`0fbfcaa`

---

## 2026-09-14 13:53 — Claude Code

- **範圍**：規則文件
- **摘要**：新增 `SPECIFICATION.md` §3.1 資訊密度規則，約束新報告預設攤開的內容量（不刪數據）。D-1：摘要每條結論 ≤ 30 字（以字元計、不含 Markdown 標記），只寫結論，原因留給後續章節。D-2：「數據與分析」攤開只留結論句與核心表圖，完整數據表與過程性描述放 `<details>` 折疊，附語法範例與 GitHub 渲染需要的空行規則。D-3：解讀句改為條件性，表圖一律有表號／圖號與表說／圖說，只有違反預期或需要額外說明時才寫解讀。同步修改 §3 骨架表的摘要列與中段說明、H-6、§7 檢查清單（改寫一項、新增兩項）；`add-report` 改為指向 §3.1；`edit-report` 註明既有報告不回溯，改成折疊寫法屬灰色地帶要先問；六份模板的摘要註解加 D-1，數據段加上 `<details>` 範例區塊並把「一句解讀」改為選用的解讀句。三條規則目前沒有腳本檢查，由檢查清單把關。未動任何報告正文
- **影響檔案**：`.agent/SPECIFICATION.md`、`.agent/skills/add-report/SKILL.md`、`.agent/skills/edit-report/SKILL.md`、`.agent/templates/` 六檔
- **commit**：`533a6ce`

---

## 2026-09-14 13:49 — Claude Code
...
## 2026-09-14 13:46 — Claude Code

- **範圍**：結構
- **摘要**：根目錄 README 瘦身為人類入口：只留標題、一句話簡介、「從哪裡開始讀」路由表。原本獨立的姊妹 repo 表格併入路由表一列，AI agent 列加上 CHANGELOG。原有的遷移公告、資料說明、異動紀錄規則收進 `<details>` 折疊；資料說明依 `AGENTS.md` 的現況，改為「多數自 Notion 匯出」。與 L1 總覽重複的「章節」表、「給 AI agent 與維護者」一段已刪除（改由路由表與 `AGENTS.md` 涵蓋）。三大模組說明原文搬到 L1 章導覽的「總覽」段，放在章節表之前，沒有放進各章章旨，以免章旨超過 3 行。README 與 `AGENTS.md` 同步加上權責說明：agent 負責結構與命名的機械性維護，也可起草報告，但敘述與結論須經人類核准。`GEMINI.md` 刪除 settings.json 提示，收斂為與 `CLAUDE.md` 相同的純轉介檔。未動任何 L2 報告正文
- **影響檔案**：`README.md`、`植物病蟲害辨識 (115資工四A)/README.md`、`AGENTS.md`、`GEMINI.md`
- **commit**：`1a83e0a`
