...
「一葉知病 OneLeaf」專題的**研究報告庫**（`OneLeaf-dx/report`），不是程式專案。內容是 36 篇 Markdown 報告 + 50 張圖表：多數由 Notion 匯出後整理而成，2026-09 起新增的報告則是直接寫在本庫。沒有建置流程、沒有測試套件；這裡的「正確性」指的是**結構規範**與**連結完整性**。

本庫由人類與 AI agent 共同維護：agent 負責結構與命名的機械性維護，也可依人類提供的數據起草報告；研究內容的敘述與結論須經人類核准後才合併。這句與 [`README.md`](README.md) 對外的說明一致，改一邊就要同步改另一邊。
