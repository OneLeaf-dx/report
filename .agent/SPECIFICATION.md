...
| L0 | `SUMMARY.md` | 成果總覽 | **只摘錄**：從 L2 報告摘錄核心結論、關鍵數字與核心表圖，每段附原報告連結；不新增論述、不跨報告歸納，內容須經人類核准 |
...
2. 圖片一律放在該章 `Image/` 底下與報告同名的資產資料夾，不共用、不跨篇引用。多篇需要同一張圖時，改為在文中連結到原報告章節。唯一例外是 L0 的 `SUMMARY.md`：可以直接引用 L2 資產資料夾內的圖，但不得複製圖檔。
...
| **TAAI2026** | TAAI 2026 投稿論文：LaTeX 原始檔、重繪論文圖的程式、論文用圖（`Image/<篇名>/`）與修訂說明 | 實驗記錄與數據本身（留在各建置章與〈系統測試與評估〉，本章只引用） |
...
| 4 | `## 1. 摘要` | 必要 | 3–5 條結論（每條 ≤ 30 字，見 §3.1 D-1）+ 一張核心數據表（設計／說明類的數據表選用） |
| 5 | 中段 | 依類型 | 見下表；段落名稱與順序以模板為準 |
| 6 | `## N. 結論與限制` | 必要 | 最後一個 H2；須含「本報告未涵蓋」一小段 |

中段不論分幾節，都是先表後圖；每張表圖需有表號／圖號與表說／圖說。哪些內容攤開、哪些折疊，以及何時寫解讀句，依 §3.1。
...
這三條管的是**預設攤開的內容量**，不是刪減數據：完整數據仍然留在報告裡，只是不擋在結論前面。

| 編號 | 規則 |
| --- | --- |
| D-1 | **摘要每條結論 ≤ 30 字**，只寫結論、不解釋原因，原因留給後續章節。字數以字元計，含標點、英數與空白，不含 Markdown 標記（`**`、反引號） |
| D-2 | **「數據與分析」預設攤開只留結論句與核心表圖。** 完整數據表與過程性描述（逐輪紀錄、逐類明細、原始日誌、逐題紀錄）放進 `<details>` 折疊 |
| D-3 | **解讀句是條件性的。** 每張表圖都要有表號／圖號與表說／圖說；只有數據違反預期，或需要額外說明（口徑不同、不可比、異常值）時，才在表圖後加解讀句。符合預期的表圖不寫解讀 |
...
<details>
<summary>表 3-3：<完整數據表的表說></summary>

| 類別 | valid | test | pooled | ±2SE |
| --- | ---: | ---: | ---: | ---: |
| <類別代號> | 0.0000 | 0.0000 | 0.0000 | 0.0000 |

</details>
```

- `<summary>` 寫表號與表說；折疊的表照樣佔用表號流水號
- `<summary>` 那行之後、`</details>` 之前**各留一個空行**，否則 GitHub 不會把裡面的表格渲染成 Markdown
- `<details>` 內不放標題：標題被折疊藏起來，目錄錨點點了卻看不到

**適用範圍**：2026-09-14 起新增的報告。既有報告不回溯；把既有報告改成折疊寫法或縮短摘要，會動到正文的呈現，屬紅線 R1 的灰色地帶，須由使用者指名該篇才做。三條規則目前沒有腳本檢查，靠 §7 檢查清單把關。
...
| H-6 | 表與圖使用**獨立於章節編號**的流水號：`表 3-1`、`圖 3-1`，並緊接一句表說／圖說（說明它是什麼）；是否再加解讀句依 §3.1 D-3 |
...
- [ ] 具備摘要與「結論與限制」
- [ ] 摘要 3–5 條，每條 ≤ 30 字、只寫結論不寫原因（§3.1 D-1）
- [ ] 「數據與分析」攤開只留結論句與核心表圖，完整數據表與過程性描述已用 `<details>` 折疊（§3.1 D-2）
- [ ] 圖片放在章節 `Image/` 底下與報告同名的資產資料夾，檔名取自 5.3 字典，無流水號
- [ ] 每張表圖有表號／圖號與表說／圖說；只有違反預期或需額外說明時才加解讀句（§3.1 D-3）
- [ ] 超過 400 行者附目錄
- [ ] md 之間以相對路徑互連，連結可點開
